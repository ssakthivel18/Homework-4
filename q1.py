...
Original file is located at
    https://colab.research.google.com/drive/1fziX8XFzND5OX8Yg6rsSFyztDPsph7OY
"""
...
def accuracy_score(y_pred, y_true):
    ...
    accuracy = (np.count_nonzero(y_pred == y_true)) / len(y_pred)
    return accuracy
...
def train_test_split(X, y, test_size):
    ...
    #Break dataset into training/test sets
    X_test, X_train = np.split(X, [int(test_size*len(X))])
    y_test, y_train = np.split(y, [int(test_size*len(y))])
...
def k_fold_cv(X, y, k_folds=5, shuffle=True):
    ...
    for i in range(k_folds):
        start = i * fold_size
        end = (i + 1) * fold_size if i != k_folds - 1 else X.shape[0]
        X_test, y_test = X[start:end], y[start:end]
        X_train, y_train = np.concatenate([X[:start], X[end:]], axis=0), np.concatenate([y[:start], y[end:]], axis=0)
