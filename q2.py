...
class LinearRegression:
    ...
    def mean_squared_gradient(self, X_batch, y_batch):
        ...
        dldw = np.dot(X_batch.T, error) / batch_size
        dldb = np.sum(error) / batch_size
        dldb = dldb.reshape(1, 1)
    
        assert dldw.shape == (D, 1), f"dldw shape not {D, 1}, but it is {dldw.shape}"
        assert dldb.shape == (1, 1), f"dldb shape not {1, 1}, but it is {dldb.shape}"
