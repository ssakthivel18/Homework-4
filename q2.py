...
Original file is located at
    https://colab.research.google.com/drive/1eY1WZHLZ2lsrnnBZLbLCg6m2b4xWtIvO
"""
...
class LinearRegression:
    ...
    def fit(self, X, y, epochs=10, lr=1e-3, batch_size=32):
        ...
        for e in range(epochs + 1): 
            ...
            for i in range(0,N + 1,batch_size): 
                if N - i >= batch_size:
                    loss = self.mean_squared_error(X[i:i + batch_size][:], y[i:i + batch_size][:])
                    dldw, dldb = self.mean_squared_gradient(X[i:i + batch_size][:], y[i:i + batch_size][:])
                    self.theta -= lr * dldw
                    self.bias -= lr * dldb
                else:
                    loss = self.mean_squared_error(X[i:][:], y[i:][:])
                    dldw, dldb = self.mean_squared_gradient(X[i:][:], y[i:][:])
                    self.theta -= lr * dldw
                    self.bias -= lr * dldb
    ...
    def mean_squared_gradient(self, X_batch, y_batch):
        ...
        dldw = 2*np.dot(X_batch.T, error) / batch_size
        dldb = 2*np.sum(error) / batch_size
